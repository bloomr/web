  ...
  describe 'POST #create' do
    before do
      expect(Mailchimp).to receive(:send_notif_about_bloomeur)
        .with('loulou@lou.com', 'loulou', 'job')
      post :create, user: {
        email: 'loulou@lou.com', job_title: 'job', first_name: 'loulou' }
    end
    ...
    it 'send a link to typeform' do
      mail = ActionMailer::Base.deliveries.last
      real_typeform_link = mail.to_s[/(https:\/\/bloomr.*)" target/m, 1]
                           .gsub("\r\n", '')
